...
def extract_data_from_directory(directory_name, file_basename, num_images, training = True, flip_left_right = False, rot_90 = False):
    imgs = []
    ...
    if rot_90:
        for i in range(num_images):
            imgs.append(tf.image.rot90(imgs[i]))
    num_images = len(imgs)
    img_patches = [img_crop(imgs[i], IMG_PATCH_SIZE, IMG_PATCH_SIZE) for i in range(num_images)]
    data = [img_patches[i][j] for i in range(len(img_patches)) for j in range(len(img_patches[i]))]
    return numpy.asarray(data)
            
def extract_train_data(num_images, flip_left_right = False, rot_90 = False):
    return extract_data_from_directory(training_data_directory,"satImage", num_images, True, flip_left_right, rot_90)
...
def extract_labels(filename, num_images, flip_left_right = False, rot_90 = False):
